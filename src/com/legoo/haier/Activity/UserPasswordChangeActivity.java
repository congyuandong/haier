...
public class UserPasswordChangeActivity extends NavigationActivity
{
	...
	private void initView() 
	{
		...
		_textAccount.setText(Hospital.getInstance().getUser().getCurrent().getAccount());
	...
	private boolean checkInput()
	{
		...
		{
			Hospital.getInstance().getAnimation().startShake(_textOld);
			return false;
		}
		else if (_textNew.getText().length() == 0)
		{
			Hospital.getInstance().getAnimation().startShake(_textNew);
			return false;
		}
