...
public class UserRegisterActivity extends NavigationActivity
{
	...
	private boolean checkInput()
	{
		...
		{
			Hospital.getInstance().getAnimation().startShake(_textAccount);
			return false;
		}
		else if (_textAccount.getText().length() < 5 || _textAccount.getText().length() > 16)
		{
			Hospital.getInstance().getToast().show(getString(R.string.toast_account_long_error));
			Hospital.getInstance().getAnimation().startShake(_textAccount);
			return false;
		}
		...
		{
			Hospital.getInstance().getAnimation().startShake(_textPassword);
			return false;
		}
		else if (_textPassword.getText().length() < 5 || _textPassword.getText().length() > 16)
		{
			Hospital.getInstance().getToast().show(getString(R.string.toast_password_long_error));
			Hospital.getInstance().getAnimation().startShake(_textPassword);
			return false;
		}
		else if (_textConfirm.getText().toString().equals(_textPassword.getText().toString()) == false)
		{
			Hospital.getInstance().getToast().show(getString(R.string.toast_password_confirm_error));
			Hospital.getInstance().getAnimation().startShake(_textConfirm);
			return false;
		}
		else if (_textTelephone.getText().length() == 0)
		{
			Hospital.getInstance().getAnimation().startShake(_textTelephone);
			return false;
		}
		else if (_textTelephone.getText().length() < 11)
		{
			Hospital.getInstance().getToast().show(getString(R.string.toast_telephone_toshort));
			Hospital.getInstance().getAnimation().startShake(_textTelephone);
			return false;
		}
		else if (_textEmail.getText().length() == 0)
		{
			Hospital.getInstance().getAnimation().startShake(_textEmail);
			return false;
		}
		else if (FormatUtils.isEmail(_textEmail.getText().toString()) == false)
		{
			Hospital.getInstance().getToast().show(getString(R.string.toast_email_error));
			Hospital.getInstance().getAnimation().startShake(_textEmail);
			return false;
		}
		...
		{
			Hospital.getInstance().getToast().show(getString(R.string.toast_idcard_error));
			Hospital.getInstance().getAnimation().startShake(_textIDCard);
			return false;
		}else if (!_checkAgreement.isChecked())
		{
			Hospital.getInstance().getToast().show(getString(R.string.toast_register_agreement));
			Hospital.getInstance().getAnimation().startShake(_checkAgreement);
		}
		return true;
	}
