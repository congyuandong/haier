...
import com.legoo.haier.Model.ContactsListModel;
...
public class FeedbackDetailActivity extends NavigationActivity
{
	...
	private ImageButton _buttonContactsList;
	...
	private TaskArchon _taskContacts;

	private ContactsListModel _contactsListModel;
...
//			Hospital.getInstance().getToast().show(_textOpinion.getText().length()+"");
			...
			{
				Hospital.getInstance().getAnimation().startShake(_textName);
				return false;
			}
			else if (_textIDCard.getText().length() == 0)
			{
				Hospital.getInstance().getAnimation().startShake(_textIDCard);
				return false;
			}
			else if (_textOpinion.getText().length()==0)
			{
				Hospital.getInstance().getAnimation().startShake(_textOpinion);
				return false;
			}
			else if (_textSuggestion.getText().length()==0)
			{
				Hospital.getInstance().getAnimation().startShake(_textSuggestion);
				return false;
			}
			else if (_textOpinion.getText().length() >= HospitalSettings.FEEDBACK_MAX_LONGTH)
			{
				Hospital.getInstance().getToast().show(getString(R.string.toast_data_long_error));
				Hospital.getInstance().getAnimation().startShake(_textOpinion);
				return false;
			}
			else if (_textSuggestion.getText().length() >= HospitalSettings.FEEDBACK_MAX_LONGTH)
			{
				Hospital.getInstance().getToast().show(getString(R.string.toast_data_long_error));
				Hospital.getInstance().getAnimation().startShake(_textSuggestion);
				return false;
	...
	private void initView() 
	{
		...
		_buttonContactsList = (ImageButton) findViewById(R.id.buttonMainNavigationContacts);
	...
	{
        _taskContacts = new TaskArchon(this,TaskArchon.ACCESS_TYPE_GET);
        _taskContacts.setWaittingEnabled(true);
        _taskContacts.setOnLoadedListener(new OnLoadedListener()
		{
			@Override
			public void OnLoaded(JsonEvent event) 
			{
				_contactsListModel = (ContactsListModel) ((ModelEvent) event).getModel();
				initEvent();
			}
		});
        _taskContacts.setOnConfirmListener(new OnConfirmListener() 
		{
			@Override
			public void onConfirm() 
			{
        ...
        _taskContacts.setOnCancelListener(new OnCancelListener()
		{
			@Override
