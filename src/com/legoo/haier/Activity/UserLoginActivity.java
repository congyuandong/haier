...
public class UserLoginActivity extends NavigationActivity
{
	...
	private void initView() 
	{
		...
		_taskArchon.setOnLoadedListener(new OnLoadedListener() 
		{
			...
			{
				if (Hospital.getInstance().getUser().login((UserModel) ((ModelEvent) event).getModel()) == true)
				{
					switch (_target)
					{
		...
		if (Hospital.getInstance().getUser().hasPrepared() == true)
		{
			_textAccount.setText(Hospital.getInstance().getUser().getRemember().getAccount());
			_textPassword.setText(Hospital.getInstance().getUser().getRemember().getPassword());
		}
