...
public class NavigationArchon 
{
	...
	{
		Hospital.getInstance().getAnimation().startProgress(_imageProgress);
		Hospital.getInstance().getAnimation().startFadeIn(_layoutProgress);
	}
	
	public void hideProgress()
	{
		Hospital.getInstance().getAnimation().startFadeOut(_layoutProgress);
	}
		...
		if (_buttonCover != null)
		{
			_buttonCover.setOnTouchListener(new OnTouchListener() 
			{
				...
				public boolean onTouch(View v, MotionEvent event) 
				{
					...
					{
						Hospital.getInstance().getAnimation().startFadeIn(_viewNavigation);
					}
					else if (event.getAction() == MotionEvent.ACTION_UP)
					{
						Hospital.getInstance().getAnimation().startFadeOut(_viewNavigation);
					}
					return false;
				}
			});
