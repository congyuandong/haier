...
public abstract class ProtoListView extends ListView 
{
	...
	public void showLoadingFooter()
	{
		...
		if (getFooterViewsCount() == 0) 
        {
			...
			Haier.getInstance().getAnimation().startProgress((ImageView)listFooterView.findViewById(R.id.imageListItemProgress));
			addFooterView(listFooterView); 
			footerType = FOOTER_LOADING;
			isFooterShowing = true;
			Haier.getInstance().getAnimation().startFade(listFooterView);
        }
	}
	...
	public void animationChild(int position)
	{
		...
		if (child != null)
		{
			Haier.getInstance().getAnimation().startFade(child);
			child = null;
		}
	}
